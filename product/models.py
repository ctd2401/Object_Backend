        ...
        try:
            if self.id:  # Check if the instance already exists (i.e., not a new object)
                ...
                if original_instance.image != self.image:
                    loggerP.info(f"prepare to update image for Product {original_instance.id} : {original_instance.image.url}")
            ...
            if self.image:
                loggerP.info(f"current image for Product {self.id} : {self.image.url}")
        except Exception as e:
            loggerP.warning(f"error : {e}") 
            transaction.set_rollback(True)
    
    def delete(self):
        loggerP.info(f"prepare to delete image for Product {self.id} : {self.image.url}")
...
class ProductVariant(models.Model):
    ...
    def save(self, *args, **kwargs):
        try:
            if self.id:  # Check if the instance already exists (i.e., not a new object)
                ...
                if original_instance.image != self.image:
                    loggerPV.info(f"prepare to update image for Product Variant {original_instance.id} Product:{original_instance.product.id} : {original_instance.image.url}")
                    if original_instance.image:
                        destroy(parse_path(original_instance.image.url), invalidate=True)
                    else:
                        pass
            ...
            super().save(*args, **kwargs)
            if self.image:
                loggerPV.info(f"current image for Product Variant {self.id} Product:{self.product.id} : {self.image.url}")
        except Exception as e:
            loggerPV.warning("error :{e}")
            transaction.set_rollback(True)
    ...
    def delete(self):
        loggerPV.info(f"prepare to delete image for Product Variant {self.id} Product:{self.product.id} : {self.image.url}")
        destroy(parse_path(self.image.url), invalidate=True)
        return super().delete()